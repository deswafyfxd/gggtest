        ...
        if not language or not country:
            ...
            if not language:
                with contextlib.suppress(ValueError):
                    lang_obj = pycountry.languages.get(name=currentLocale[0].split("_")[0])
                    if lang_obj:
                        language = lang_obj.alpha_2
                    else:
                        logging.warning(f"Could not find language: {currentLocale[0].split('_')[0]}")
            if not country:
                with contextlib.suppress(ValueError):
                    country_obj = pycountry.countries.get(name=currentLocale[0].split("_")[1])
                    if country_obj:
                        country = country_obj.alpha_2
                    else:
                        logging.warning(f"Could not find country: {currentLocale[0].split('_')[1]}")
        if not language or not country:
            try:
